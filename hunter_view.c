...
{
	return gv_get_location(hv->game_view, player);
}
	...
	bool road, bool rail, bool sea)
{
	...
	location_t from = hv_get_location (hv, player);
	round_t round = hv_get_round (hv);
	
	return gv_get_connections (hv->game_view , n_locations, from, player, round, road, rail, sea);
}
